...
- `uv run pytest tests/ -v` — run all tests (104 tests, ~25s)
...
## Architecture
- models/ — U-Net backbone, TrackNet wrapper, WBCE loss
- data/ — dataset, heatmap generation, transforms (flip, jitter, mixup)
- training/ — trainer loop, evaluation metrics, YAML config
- inference/ — postprocess, trajectory rectification, Kalman tracker, CLI
